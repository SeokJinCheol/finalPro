...
<html>
<head>
<title>requestboardupdateForm</title>
<!-- CSS 연결-->
		...
		<c:forEach items="${updatelist}" var="updatelist">
			<form action="requestBoardUpdate" method="post">
				<table border="1">
					<tr>
						<td>글번호</td>
						<td>${updatelist.codeNum}<input type="hidden" name="codeNum" value="${updatelist.codeNum}"></td>
					</tr>
					<tr>
						<td>제목</td>
					...
					<tr>
						<td>대여시작일</td>
						<td><input type="date" name="startDate" min="${updatelist.startDate}" max="${updatelist.endDate}" value="${updatelist.startDate}" required></td>
					</tr>
					<tr>
						<td>대여종료일</td>
						<td><input type="date" name="endDate" min="${updatelist.startDate}" max="${updatelist.endDate}" value="${updatelist.endDate}" required></td>
					</tr>
					<tr>
						<td>대여비</td>
						<td><input type="text" name="bill" value="${updatelist.bill}" required>원</td>
					</tr>
					<tr>
						<td>보증금</td>
						<td><input type="text" name="deposit" value="${updatelist.deposit}" required>원</td>
					</tr>
					<tr>
						<td>내용</td>
