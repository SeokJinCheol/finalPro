...
<body>
	<div class="row" style="background: #6699DC; color: white; height: 100px; text-align: right; line-height: 130px;">
		<img src="/team4/resources/images/left-question122.png" style="margin-right: 10px;"> 
		<font style="margin-right: 30px; font-family: 'Hanna', fantasy; font-style: bold; font-size: 30px;">Q & A</font>
	</div>
	...
	<div align=center style="background: #f5f6f7; width: 1350px; height: 500px; vertical-align: middle; display: table-cell;">
		...
		<div class="w3-row" style="margin-top:50px;">
			<form action="QnA_list" method=post>
				...
				<input type="text" name="word" value="${word}" placeholder="&nbsp;Search . . . "  style= "border-color: rgb(169, 169, 169);" class="free-search-title" required="required"/>
		...
		<div style="margin-bottom: 20px; width: 73%;">
			<security:authorize ifAnyGranted="role_user, role_com, role_master">
				<div class="w3-row">
					<div class="w3-col w3-right" style="margin-right:8px; width: 10%; margin-bottom:10px;">
						<br><br>
						<input type="button" class="button-write"  value ="글 쓰 기" onclick="location='QnA_write'">
					</div>
				</div>
			</security:authorize>
			...
			<table style="padding: 70px; text-align:center; border-collapse: collapse; border-spacing: 0; padding-top: 5px; padding-bottom: 10px;">
				<tr class="bottom-border2" style=" height:30px; margin-bottom: 20px; background-color: #80b5ea; color:white; text-align: center;">
			         ...
			         <td width=60 style="border-top: 2px solid #000; border-right: 1px solid #fff;">Img</td>
			         <td width=120 style="border-top: 2px solid #000; border-right: 1px solid #fff;">Category</td>
				...
				<form name="goLink">
					...
					<c:forEach items="${list}" var="list">
			        	<tr>
			            	...
			            	<td style="border-bottom: 3px solid #fff; width: 10%; "><img src="/team4/resources/QandAImg/${list.img }" style="width: 100%; height: 50px;"></td>
							<td style="border-bottom: 3px solid #fff;">${list.category}</td>
			             	...
			             	<c:choose>
			             		<c:when test="${fn:length(list.title)>15 }">
			             			...
			             			<security:authorize ifAnyGranted="role_master">
										<td style="border-bottom: 3px solid #fff;">
											<a href="javascript:void(0)" onclick="move(${list.bnum});">
												<c:out value="${fn:substring(list.title,0,14)}" />...
									...
									<security:authorize ifNotGranted="role_master, role_user, role_com">
										<td style="border-bottom: 3px solid #fff;">
											<c:out value="${fn:substring(list.title,0,14)}" />...
											<c:if test="${list.replyCount != 0}">(${list.replyCount})</c:if>
										</td>
				...
				<c:if test="${count>0}">
					...
					<ul>
						<c:if test="${startPage>5}">
							<c:if test="${word == null}">
								<a style="text-decoration:none;" href="QnA_list?pageNum=${startPage-5}">&laquo;</a>
							</c:if>
							<c:if test="${word != null && keyword != null}">
								<a style="text-decoration:none;" href="QnA_list?pageNum=${startPage-5}&word=${word}&keyword=${keyword}">&laquo;</a>
							</c:if>
						</c:if>
						...
						<table>
							<c:forEach var="i" begin="${startPage}" end="${endPage}">
								...
								<c:if test="${word != null }">
									<td style="background: #6699DC; color:white; width:20px; text-align: center;border-radius: 5px;">
										<a style="text-decoration:none;" href="QnA_list?pageNum=${i}&word=${word}&keyword=${keyword}">${i}</a>
									</td>
								</c:if>
							</c:forEach>
						</table>
						...
						<c:if test="${endPage<pageCount}">
							<c:if test="${word == null}">
								<a style="text-decoration:none;" href="QnA_list?pageNum=${startPage+5}">&raquo;</a>
							</c:if>
	
							<c:if test="${word != null }">
								<a style="text-decoration:none;" href="QnA_list?pageNum=${startPage+5}&word=${word}&keyword=${keyword}">&raquo;</a>
							</c:if>
						</c:if>
					</ul>
					<br>
