			...
			<c:forEach items="${result}" var="result">
				<c:if test="${result.bNum==result.bgnum }">
					...
					<input type="button" value="답글달기"
						onclick="window.location='insertForm?bNum=${result.bNum}&bgnum=${result.bgnum }&groupnum=${result.groupnum}&ranknum=${result.ranknum} '">
					<input type="button" value="수정하기"
						onclick="window.location='updateForm?bNum=${result.bNum}&bgnum=${result.bgnum }&id=${result.id } '">
					<input type="button" value="삭제하기"
						onclick="window.location='deleteForm?bNum=${result.bNum}&bgnum=${result.bgnum }&id=${result.id } '">
				</table>
				<br> <br>
				...
				<form action="insertForm" method="post">
					<table border=0>
						...
						<tr>
							<td width="500"><input type="text" name="contents"
								style="width: 100%; height: 50px;" align="left"></td>
						</tr>
